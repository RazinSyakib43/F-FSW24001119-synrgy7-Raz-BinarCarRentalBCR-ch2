...
export function CarProvider({ children }: { children: React.ReactNode }) {
    ...
    const searchCars = async (driverType: string, capacity: string) => {
        try {
            const response = await axios.get(`https://itchy-kass-zeens-e78e4aa6.koyeb.app/api/v1/client/cars`, {
    ...
    const fetchCarsForDashboard = async () => {
        try {
            const response = await axios.get('https://itchy-kass-zeens-e78e4aa6.koyeb.app/api/v1/dashboard/cars/dashboard', {
                headers: {
    ...
    const getCarDetailById = async (id: string) => {
        try {
            const response = await axios.get(`https://itchy-kass-zeens-e78e4aa6.koyeb.app/api/v1/dashboard/cars/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
                }
            });
    ...
    const addCar = async (formData: FormData) => {
        try {
            const response = await axios.post('https://itchy-kass-zeens-e78e4aa6.koyeb.app/api/v1/dashboard/cars', formData, {
                headers: {
    ...
    const updateCar = async (id: string, formData: FormData) => {
        try {
            const response = await axios.put(`https://itchy-kass-zeens-e78e4aa6.koyeb.app/api/v1/dashboard/cars/${id}`, formData, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')?.replace(/"/g, '')}`,
                    'Content-Type': 'multipart/form-data'
                }
    ...
    const deleteCar = async (id: string) => {
        try {
            const response = await axios.delete(`https://itchy-kass-zeens-e78e4aa6.koyeb.app/api/v1/dashboard/cars/${id}`, {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('token')?.replace(/"/g, '')}`,
                }
            });
