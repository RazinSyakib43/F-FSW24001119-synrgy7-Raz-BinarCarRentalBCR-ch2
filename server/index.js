...
const prepareFile = async (url) => {
  ...
  if (!url.startsWith("/node_modules")) {
    if (url.startsWith("/styles/")) {
      paths[0] += "/style"; // mengubah jalur untuk melayani file CSS dari folder 'style'
    } else {
      paths[0] += "/public"; // jika url bukan /node_modules maka path akan ditambahkan '/public' agar bisa membaca file di folder public
    }
  }
...
http
  .createServer(async (req, res) => {
    if (req.url === "/cars") {
      // routing bila di browser membuka link /cars maka akan membuka file search-cars.html
